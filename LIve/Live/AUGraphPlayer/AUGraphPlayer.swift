...
extension AUGraphPlayer {
    ...
    func initializePlayGraph() -> Bool {
        ...
        stereoStreamFormat.mBitsPerChannel = UInt32(8 * bytesPerSample)
        stereoStreamFormat.mSampleRate = 48000.0
        
        guard
            check(AudioUnitSetProperty(mPlayerIOUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Output, 1, &stereoStreamFormat, UInt32(MemoryLayout.stride(ofValue: stereoStreamFormat))), "Set remote IO output element stream format"),
            check(
                AudioUnitSetProperty(mPlayerUnit,
                                     kAudioUnitProperty_StreamFormat,
                                     kAudioUnitScope_Output,
                                     0,
                                     &stereoStreamFormat,
                                     UInt32(MemoryLayout.stride(ofValue: stereoStreamFormat))),
                "Could not Set StreamFormat for Player Unit") else {
                    return false
                }
        
        //配置Splitter属性
        status =  AudioUnitSetProperty(mSplitterUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Output, 0, &stereoStreamFormat, UInt32(MemoryLayout.stride(ofValue: stereoStreamFormat)))
        guard check(status, "Could not Set StreamFormat output for Splitter Unit") else {
            return false
        }
        status =  AudioUnitSetProperty(mSplitterUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, 0, &stereoStreamFormat, UInt32(MemoryLayout.stride(ofValue: stereoStreamFormat)))
        guard check(status, "Could not Set StreamFormat input for Splitter Unit") else {
            return false
        }
        //配置VocalMixerUnit的属性
        status =  AudioUnitSetProperty(mVocalMixerUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Output, 0, &stereoStreamFormat, UInt32(MemoryLayout.stride(ofValue: stereoStreamFormat)))
        guard check(status, "Could not Set StreamFormat output for VocalMixer Unit") else {
            return false
        }
        status =  AudioUnitSetProperty(mVocalMixerUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, 0, &stereoStreamFormat, UInt32(MemoryLayout.stride(ofValue: stereoStreamFormat)))
        ...
        //配置AccMixerUnit的属性
        status = AudioUnitSetProperty(mAccMixerUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Output, 0, &stereoStreamFormat, UInt32(MemoryLayout.stride(ofValue: stereoStreamFormat)))
        guard check(status, "Could not Set StreamFormat output for mAccMixerUnit") else {
            return false
        }
        status = AudioUnitSetProperty(mAccMixerUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, 0, &stereoStreamFormat, UInt32(MemoryLayout.stride(ofValue: stereoStreamFormat)))
        ...
        guard
            check(status, "Connect mPlayerNode mSplitterNode failed")
        else { return false }
...
extension AUGraphPlayer {
    ...
    func setupFilePlayer() -> Bool {
        ...
        var fileASBD:AudioStreamBasicDescription? = nil
        ...
        timeStamp.mFlags = .smpteTimeValid
        timeStamp.mSampleTime = 0
        var rgn = ScheduledAudioFileRegion.init(mTimeStamp: timeStamp, mCompletionProc: nil, mCompletionProcUserData: nil, mAudioFile: musicFile! , mLoopCount: 0, mStartFrame: 0, mFramesToPlay: UInt32(nPackets) * (fileASBD?.mFramesPerPacket ?? 0))
        ...
        startTime.mFlags = .smpteTimeValid
        startTime.mSampleTime = -1.0
